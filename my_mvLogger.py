...
class myMvLogger:
    ...
    def __init__(self, pos_x, pos_y, mylabel, width, height, no_move, no_resize):

        self.log_level = 0
        self._auto_scroll = True
        self.filter_id = None
        self.window_id = dpg.add_window(
            label=mylabel, pos=(pos_x, pos_y), width=width, height=height, no_title_bar=True, no_move=no_move, no_resize=no_resize)
        self.count = 0
        ...
        with dpg.group(horizontal=True, parent=self.window_id):
            ...
            dpg.add_button(label="Clear", callback=lambda: dpg.delete_item(
